...
impl<F: PrimeField, MM: MarlinMode> AHPForR1CS<F, MM> {
    ...
    fn index_helper<C: ConstraintSynthesizer<F>>(c: &C) -> Result<IndexerState<F>, AHPError> {
        ...
        let evals: Vec<_> = {
            #[cfg(not(feature = "serial"))]
                let iter = [(&a, &non_zero_a_domain), (&b, &non_zero_b_domain), (&c, &non_zero_c_domain),].into_par_iter();

            #[cfg(feature = "serial")]
                let iter = [(&a, &non_zero_a_domain), (&b, &non_zero_b_domain), (&c, &non_zero_c_domain),].into_iter();
